...
export const useInversionista = () => {
    ...
    const obtenerInversionistas = async() => {
        ...
        let respond = await prestamoApi.get('/api/inversionistas'+dataParam,config);
        ...
        {
            inversionistas.value = jwtDecode(respond.data).inversionistas;
        }
    }

    const listar = async(page=1) => {
        dato.value.page = page
        await obtenerInversionistas();
    }
    ...
    const isActived = () => {
        return inversionistas.value.current_page;
    }

    const pagesNumber = () => {
        if(!inversionistas.value.to){
            return []
        }

        let from = inversionistas.value.current_page - offest.value;

        if(from < 1){ from = 1; }

        let to = from + ( offest.value*2 );

        if( to >= inversionistas.value.last_page){ to = inversionistas.value.last_page; }
    ...
    const obtenerListaSuperioresPorRole = async(data) => {
        let respond = await prestamoApi.get('/api/inversionistas/superiores-por-role?role='+data,config)
    ...
    const obtenerInversionista = async(id) => {
        ...
        let respond = await prestamoApi.get('/api/inversionistas/show?id='+id,config);

        inversionista.value = jwtDecode(respond.data).inversionista
    }
    ...
    const agregarInversionista = async(data) => {
        errors.value = [];
        respuesta.value = [];
        try {
            let respond = await prestamoApi.post('/api/inversionistas',data,configPost);
    ...
    const actualizarInversionista = async(data) => {
        errors.value = ''
        try {
            let responded = await prestamoApi.post('api/inversionistas/update',data,configPost)
    ...
    const inhabilitarInversionista = async(id) => {
        errors.value = ''
        try {
            let responded = await prestamoApi.put('api/inversionistas/'+id+'/disable',null,configPost)
            errors.value =''
            responded = jwtDecode(responded.data)
            if(responded.ok==1){
                respuesta.value=responded
    ...
    const habilitarInversionista = async(id) => {
        errors.value = ''
        try {
            let responded = await prestamoApi.put('api/inversionistas/'+id+'/enable',null,configPost)
            errors.value =''
            responded = jwtDecode(responded.data)
            if(responded.ok==1){
                respuesta.value=responded
    ...
    const subirContratoEmpleado = async(data) => {
        errors.value = []
        try {
            let responded = await prestamoApi.post('api/inversionistas/subir-contrato',data,configUploadPost);
    ...
    const verContratoEmpleado = async(id) => {
        let emple = inversionistas.value.data.find(e => e.id === id);
        if(emple)
        {
            let contrato_empleado = emple.contrato_pdf;
            let dni_empleado = emple.numero_documento;
            let url = import.meta.env.VITE_APP_API_URL+"/storage/inversionistas/"+dni_empleado+'/'+contrato_empleado;
            window.open(url,'_blank')
        }
    }
    
    return {
        errors, respuesta, inversionista, inversionistas, dato, form, tipoDocumentos, sexos,roles, persona,
        departamentos, provincias, distritos, superiores,
        listar, obtenerInversionistas, buscar, isActived, pagesNumber,
        ...
        obtenerInversionista,obtenerListaRoles, buscarDatosDni, agregarInversionista, actualizarInversionista ,
        habilitarInversionista, inhabilitarInversionista, imprimirContratoEmpleado,subirContratoEmpleado,
        verContratoEmpleado
    }
