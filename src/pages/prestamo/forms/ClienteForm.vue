...
const limpiarCliente = () => {
    ...
    clienteFrm.value.departamento_id="";
    clienteFrm.value.provincia_id="";
    clienteFrm.value.distrito_id="";
...
const {
    ...
    obtenerListaTipoDocumentos, obtenerListaSexos, obteneListaDepartamentos,
    obteneListaProvincias, obteneListaDistritos,
...
const obtenerPersona = async() => {
    ...
    if(persona.value.numero_documento)
    {
        ...
        clienteFrm.value.sexo_id = persona.value.sexo_id;
        clienteFrm.value.telefono = persona.value.telefono;
        clienteFrm.value.direccion = persona.value.direccion;
        clienteFrm.value.departamento_id = persona.value.departamento_id;
        clienteFrm.value.provincia_id = persona.value.provincia_id;
        clienteFrm.value.distrito_id = persona.value.distrito_id;
        buscandoCliente.value= false
    }
    // if(persona.value)
    // {
...
const listarProvincias = async() => {
    await obteneListaProvincias(clienteFrm.value.departamento_id);
}

const listarDistritos = async() => {
    await obteneListaDistritos(clienteFrm.value.provincia_id);
}
            ...
            <div class="form-group row">
                ...
                <div class="col-md-8 mb-1">
                    <textarea class="form-control form-control-sm" id="direccion"
                        ...
                        :class="{ 'is-invalid' : clienteFrm.errors.direccion}" placeholder="Ingrese Dirección"
                    ></textarea>
                    <small class="text-danger" v-for="error in clienteFrm.errors.direccion" :key="error">{{error }}</small>
                </div>
            </div>
            <div class="form-group row">
                <label for="depatamento" class="col-form-label col-form-label-sm col-md-4 mb-1">Departamento:</label>
                <div class="col-md-8 mb-1">
                    <select class="form-control form-control-sm"
                        v-model="clienteFrm.departamento_id" id="depatamento"
                        :class="{ 'is-invalid' : clienteFrm.errors.departamento_id}"
                        @change="listarProvincias()">
                        <option value="">-Seleccionar-</option>
                        <option v-for="depa in departamentos" :value="depa.id" >{{ depa.nombre }}</option>
                    </select>
                    <small class="text-danger" v-for="error in clienteFrm.errors.departamento_id" :key="error">{{error }}</small>
                </div>
            </div>
            <div class="form-group row">
                <label for="provincia" class="col-form-label col-form-label-sm col-md-4 mb-1">Provincia:</label>
                <div class="col-md-8 mb-1">
                    <select class="form-control form-control-sm"
                        v-model="clienteFrm.provincia_id" id="provincia"
                        :class="{ 'is-invalid' : clienteFrm.errors.provincia_id}"
                        @change="listarDistritos">
                        <option value="">-Seleccionar-</option>
                        <option v-for="prov in provincias" :value="prov.id" >{{ prov.nombre }}</option>
                    </select>
                    <small class="text-danger" v-for="error in clienteFrm.errors.provincia_id" :key="error">{{error }}</small>
                </div>
            </div>
            <div class="form-group row">
                <label for="distrito" class="col-form-label col-form-label-sm col-md-4 mb-1">Distrito:</label>
                <div class="col-md-8 mb-1">
                    <select class="form-control form-control-sm"
                        v-model="clienteFrm.distrito_id" id="distrito"
                        :class="{ 'is-invalid' : clienteFrm.errors.distrito_id}">
                        <option value="">-Seleccionar-</option>
                        <option v-for="dist in distritos" :value="dist.id" >{{ dist.nombre }}</option>
                    </select>
                    <small class="text-danger" v-for="error in clienteFrm.errors.distrito_id" :key="error">{{error }}</small>
                </div>
            </div>
        </div>
    </div>
